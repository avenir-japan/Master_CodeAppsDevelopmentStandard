...
export function normalizeVersionSpec(spec) {
  ...
  const match = spec
    .trim()
    .match(
      /^(?:workspace:)?[~^]?([0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?(?:-[0-9A-Za-z.-]+)?)/,
    );
  return match?.[1] ?? null;
}
...
export function parseIssueLabels(rawLabels) {
  const labels =
    typeof rawLabels === "string" ? rawLabels : DEFAULT_ISSUE_LABELS.join(",");
  return labels
    .split(",")
...
export function issueCoversNpm(issues, packageName, version) {
  ...
  return issues.some((issue) => {
    ...
    return (
      ...
      (text.includes(packageName) && versionPattern.test(text))
    );
  });
}
...
export function issueCoversUpstream(issues, sourcePath, commit) {
  ...
  return issues.some((issue) => {
    ...
    if (
      text.includes(marker) ||
      text.includes(commit.htmlUrl) ||
      text.includes(shortSha)
    )
      return true;
    return pullRequest
      ? text.includes(
          `github.com/microsoft/power-platform-skills/pull/${pullRequest}`,
        )
      : false;
  });
}
...
export async function discoverLocalVersions(repositoryRoot) {
  ...
  const versions = new Map(
    NPM_SOURCES.map((packageName) => [packageName, new Set()]),
  );
...
export function buildIssueBody(
  ...
  if (npmUpdates.length > 0) {
    ...
    for (const update of npmUpdates) {
      lines.push(
        `| [\`${update.packageName}\`](${update.url}) | \`${update.version}\` | ${formatDate(update.publishedAt)} |`,
      );
    }
  ...
  if (upstreamUpdates.length > 0) {
    ...
    for (const update of upstreamUpdates) {
      ...
      lines.push(
        `| \`${update.sourcePath}\` | [\`${update.sha.slice(0, 8)}\`](${update.htmlUrl}) | ${formatDate(update.committedAt)} | ${summary} |`,
      );
    }
    lines.push("");
  }
  ...
  for (const update of npmUpdates)
    lines.push(npmMarker(update.packageName, update.version));
  for (const update of upstreamUpdates)
    lines.push(upstreamMarker(update.sourcePath, update.sha));
...
export async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    await writeSummary([
      "Usage: node .github/scripts/check-sdk-updates.mjs [--dry-run]",
      "",
      "Required:",
      "- ISSUE_REPOSITORY or GITHUB_REPOSITORY",
      "- GH_TOKEN or GITHUB_TOKEN unless --dry-run or DRY_RUN=true",
      "",
      "Optional:",
      "- DRY_RUN=true",
      "- ISSUE_LABELS=sdk-update,enhancement",
      "- UPSTREAM_GITHUB_TOKEN=<token>",
    ]);
    return;
  }

  const repository =
    process.env.ISSUE_REPOSITORY ?? process.env.GITHUB_REPOSITORY;
  const token = process.env.GH_TOKEN ?? process.env.GITHUB_TOKEN ?? "";
  const dryRun =
    process.argv.includes("--dry-run") || process.env.DRY_RUN === "true";
  const issueLabels = parseIssueLabels(
    process.env.ISSUE_LABELS ?? DEFAULT_ISSUE_LABELS.join(","),
  );

  if (!repository)
    throw new Error("ISSUE_REPOSITORY or GITHUB_REPOSITORY is required");
  if (!token && !dryRun)
    throw new Error(
      "GH_TOKEN or GITHUB_TOKEN is required unless dry-run is enabled",
    );
  ...
  for (const packageName of NPM_SOURCES) {
    try {
      ...
      console.warn(
        `Skipping npm registry check for ${packageName}: ${error.message}`,
      );
    }
  }
  for (const sourcePath of UPSTREAM_SOURCES) {
    try {
      ...
      if (!dryRun) throw error;
      console.warn(
        `Skipping upstream GitHub check for ${sourcePath}: ${error.message}`,
      );
    }
  }

  const npmUpdates = npmLatest.filter((latest) => {
    const adoptedLocally = localVersions
      .get(latest.packageName)
      ?.has(latest.version);
    return (
      !adoptedLocally &&
      !issueCoversNpm(issues, latest.packageName, latest.version)
    );
  });
  ...
  await writeSummary([
    "## SDK update check",
    "",
    `Issue を作成しました: [#${issue.number}](${issue.html_url})`,
  ]);
}
