...
共通認証、`.env`、ソリューション前提の正本は [standard スキル](../standard/SKILL.md) とし、Dataverse ではその `auth_helper.py` を利用する。

- 共通実装: `.github/skills/standard/scripts/auth_helper.py`
- 案件側で必要なら `auth_helper.py` をプロジェクト直下へコピーして使う
- 詳細 API と 2 層キャッシュ構成は [認証リファレンス](../standard/references/auth-patterns.md) を参照

> **Dataverse で特に重要**: `api_get()` は `dict` を直接返す。`.json()` を呼ばない。
...
| `dataverse`        | テーブル構築後に同スキル内のセキュリティロール reference を参照 |
