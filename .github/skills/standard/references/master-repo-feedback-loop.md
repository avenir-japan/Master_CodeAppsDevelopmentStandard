...
- GitHub へ commit / push する場合は、更新箇所の説明、変更要約、ユーザー向け報告を日本語で整理する
...
更新箇所の説明文は日本語で記載してください。
```

### テンプレート D: 会話ログから細かな還元候補を拾う

```text
ソース案件フォルダは {案件フォルダ名またはパス} です。
加えて、対象会話ログは {session ID / transcript パス / 期間 / 機能範囲} です。

この会話ログと案件 repo を突き合わせて、マスターへ戻せそうな「細かい内容」を洗い出してください。
特に、命名規則、失敗時のフォールバック判断、削除安全策、通知表示の細則、検証手順のような、成果物だけでは埋もれやすい知見を優先してください。

まずは次の 4 区分で整理してください。
1. 既にマスターへ反映済み
2. 未反映で戻す候補
3. 抽象化が必要な候補
4. 仮説段階または検証不足のため戻さない候補

各候補について、根拠になった会話上の決定・失敗・修正・検証結果と、戻し先候補も示してください。
まだマスター側は編集せず、私の承認待ちにしてください。
```
