...
> [!NOTE]
> 既定モデル: GPT-5.4 / 高難度タスクでは GPT 系の上位 reasoning model を第一候補、比較検討では Opus 系も候補
...
GitHub Copilot は使用量ベース課金を前提に、**既定を GPT-5.4** とし、軽作業では軽量モデル、コード中心の実装ではコード特化モデルも使い分けつつ、**深い推論が必要な場面ではまず GPT 系の上位 reasoning model を候補にし、比較検討や別観点レビューが必要な場合は Opus 系も候補**にする。

| 用途                                     | 既定モデル | 上位モデル運用                                                              |
| ---------------------------------------- | ---------- | --------------------------------------------------------------------------- |
| 通常実装、設定変更、ドキュメント更新     | GPT-5.4    | なし。まずは GPT-5.4 で完了を目指す                                         |
| コード量が多い実装、テスト追加、整形修正 | GPT-5.4    | コード特化モデルを使うと手戻りが減る場合のみ候補                            |
| アーキテクチャ判断、原因不明の不具合調査 | GPT-5.4    | まず GPT 系の上位 reasoning model。別観点の比較検討では Opus 系も候補       |
...
| リポジトリ同梱の Copilot カスタマイズ | `.github/agents/PowerCode.agent.md`、`.github/skills/`                            | ✅                     | この標準リポジトリを開いたときに参照される                       |
...
本リポジトリには、GitHub Copilot エージェント（`PowerCode`）が利用する開発スキルが含まれる。
...
`PowerCode` エージェント（`.github/agents/PowerCode.agent.md`）が上記スキルを自動的に参照し、
